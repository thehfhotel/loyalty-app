...
-- Copied from Prisma migration to ensure compatibility between Node.js and Rust backends
-- Source: backend/prisma/migrations/0_init/migration.sql
-- This schema is identical to the Prisma migration
...
-- CreateFunction: update_coupon_analytics
-- Updates daily analytics for all coupons
...
-- CreateFunction: assign_coupon_to_survey
-- Assigns a coupon to be awarded upon survey completion
...
-- CreateFunction: remove_coupon_from_survey
-- Deactivates a coupon assignment from a survey
...
-- CreateFunction: award_survey_completion_coupons
-- Awards coupons when a survey is completed
...
-- CreateTriggerFunction: trigger_award_survey_coupons
-- Trigger function to automatically award coupons on survey completion
...
COMMENT ON FUNCTION trigger_award_survey_coupons() IS 'Trigger function to automatically award coupons on survey completion';

-- CreateTrigger: survey_completion_reward_trigger
-- Note: Only create if not exists to avoid errors on re-run
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'survey_completion_reward_trigger'
    ) THEN
