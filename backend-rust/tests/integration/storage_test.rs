...
/// Upload avatar image and verify avatar URL is returned (converted to JPEG)
#[tokio::test]
async fn test_upload_avatar() {
    // Arrange
    let (router, _temp_dir) = create_test_storage_router();

    // Create a valid 1x1 PNG test image (will be converted to JPEG by image processing)
    let png_data: &[u8] = &[
        0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44,
        0x52, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x06, 0x00, 0x00, 0x00, 0x1F,
        0x15, 0xC4, 0x89, 0x00, 0x00, 0x00, 0x0A, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9C, 0x63, 0x00,
    ...
    // All avatars are converted to JPEG
    assert!(
        avatar_url.unwrap().ends_with(".jpg"),
        "Avatar should be saved as JPEG: {}",
        avatar_url.unwrap()
    );
...
/// Test uploading avatar with non-image data (should be rejected by image decoder)
#[tokio::test]
async fn test_upload_avatar_invalid_content_type() {
    ...
    // Assert - Should return 400 (image decoder rejects non-image data)
    assert!(
        test_response.status == StatusCode::BAD_REQUEST
            || test_response.status == StatusCode::UNSUPPORTED_MEDIA_TYPE,
        "Expected 400 or 415 status for non-image avatar, got {:?}. Body: {}",
        test_response.status,
        test_response.body
    );
}
