-- Fix Tier System Reset Migration
-- This migration completely resets the tier system to correct the progression issues

-- Step 1: Disable all existing tiers
UPDATE tiers SET is_active = false WHERE is_active = true;

-- Step 2: Delete all existing tier data to start fresh
DELETE FROM tiers WHERE is_active = false;

-- Step 3: Insert correct night-based tiers with proper values
...
-- Step 4: Ensure user_loyalty table has total_nights column
ALTER TABLE user_loyalty ADD COLUMN IF NOT EXISTS total_nights INTEGER DEFAULT 0;

-- Step 5: Reset all user total_nights to 0 initially
UPDATE user_loyalty SET total_nights = 0;

-- Step 6: Recalculate total_nights from points_transactions
UPDATE user_loyalty 
...
-- Step 7: Drop and recreate the user_tier_info view with correct logic
...
SELECT 
    ...
    -- Backward compatibility fields for points
...
-- Step 8: Update tier assignment function
...
-- Step 10: Recalculate all user tiers based on current nights
UPDATE user_loyalty 
SET total_nights = total_nights; -- This triggers the tier update

-- Step 11: Create function for adding nights and points together
...
BEGIN
    ...
    LEFT JOIN tiers t ON t.name = uti.tier_name
...
-- Step 12: Verify the fix by checking a sample user
-- (This is just for logging/verification, no actual operation)
DO $$
DECLARE
    sample_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO sample_count FROM user_tier_info LIMIT 1;
    RAISE NOTICE 'Tier system reset completed. Sample user count: %', sample_count;
END $$;