...
const TranslationButton: React.FC<TranslationButtonProps> = ({
  ...
  const effectiveAvailableLanguages =
  ...
  const handleStartTranslation = () => {
    ...
    const untranslatedLanguages = targetLanguages.filter(lang => !effectiveAvailableLanguages.includes(lang));
  ...
  const isFirstTranslation = effectiveAvailableLanguages.length === 1;
    ...
    <div className="relative">
      ...
      >
        {isTranslating ? (
          ...
          <>
            <span data-testid="globe-icon" className="mr-2 h-4 w-4" aria-hidden="true">
              🌐
            </span>
        ...
        <div className="mt-3 bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          ...
          <div className="space-y-2 mb-4">
            {targetLanguages.map((language) => {
              const isTranslated = effectiveAvailableLanguages.includes(language);
              return (
